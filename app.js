...
class GridSystem {
    constructor() {
        ...
        this.extraArr = ["TCR", "CUR", "CT", "ELI", "LSH", "LJY", "TWN", "RYD", "JX", "LK", "JV"];
        ...
        this.playersArr = [
            this.p1 = new Player({x: 1, y: 10, lable: 13, id: this.extraArr[0], area: "area1", color: "grey", startingSteps: 500}),
    ...
    resetPosition(plyrSlot) {
        ...
        plyrSlot.x = this.team1OriginPosition[index].x;
        plyrSlot.y = this.team1OriginPosition[index].y;
        plyrSlot.area = this.team1OriginPosition[index].area;;
    ...
    translocator(plyrSlot) {
        ...
        this.matrix[10][31] = plyrSlot.lable;
        plyrSlot.x  = 31
        plyrSlot.y = 10;
        
        this.updMatrixForPlayerAtThisSpot(plyrSlot)
    }
    translocator2(plyrSlot, teamNum) {
        ...
        if (teamNum === 1) {
            const getSlotPosition = this.teamSlots1.length - 1;
            plyrSlot.x  = this.team1OriginPosition[getSlotPosition].x;
            plyrSlot.y = this.team1OriginPosition[getSlotPosition].y;
            plyrSlot.area = this.team1OriginPosition[getSlotPosition].area;
            // plyrSlot.originX  = this.team1OriginPosition[getSlotPosition].x;
            // plyrSlot.originY = this.team1OriginPosition[getSlotPosition].y;
            // plyrSlot.originArea = this.team1OriginPosition[getSlotPosition].area;
            this.matrix = this.allMatrixes[plyrSlot.area].gridMatrix;

            this.matrix[plyrSlot.y][plyrSlot.x] = plyrSlot.lable;
            
        } else if(teamNum === 2) {
            const getSlotPosition = this.teamSlots2.length - 1;
            plyrSlot.x  = this.team2OriginPosition[getSlotPosition].x;
            plyrSlot.y = this.team2OriginPosition[getSlotPosition].y;
            plyrSlot.area = this.team2OriginPosition[getSlotPosition].area;
            // plyrSlot.originX  = this.team2OriginPosition[getSlotPosition].x;
            // plyrSlot.originY = this.team2OriginPosition[getSlotPosition].y;
            // plyrSlot.originArea = this.team2OriginPosition[getSlotPosition].area;
            this.matrix = this.allMatrixes[plyrSlot.area].gridMatrix;
    ...
    setPlayerTeam(plyrSlot, team) {
        ...
        if (plyrSlot.team === "1") {
            ...
            this.teamSlots1.push(plyrSlot.id);
            this.translocator2(plyrSlot, 1)

        }
        if (plyrSlot.team === "2") {
            plyrSlot.obtainedPowers = [];
            this.teamSlots2.push(plyrSlot.id);
            this.translocator2(plyrSlot, 2)
        }
        if (plyrSlot.team === "0") {
            plyrSlot.obtainedPowers = [];
            const index = this.teamSlots1.indexOf(plyrSlot.id);
            if (index !== -1) {this.teamSlots1.splice(index, 1)};

            const index2 = this.teamSlots2.indexOf(plyrSlot.id);
...
io.sockets.on('connection', function (sock) {
    ...
    sock.on('pushPowerArray', data => {
        ...
        sock.emit('updatePowerArray', { obtainedPowers, getTeam });
    });
